...
function initvars() {
  for (var i = 1; i <= anzahl; i++) {
    ...
    if (i<4)
        ...
        Game.basPreis[i] = Decimal(Decimal.pow(Decimal.mul(i,0.25).add(0.6), 166).mul(2));  
    ...
    for (var i1 = 0; i1 < upgradeanzahl; i1++) {
        ...
        if (i<3)
            ...
            Game.upgradeBasPreis[i][i1] = Decimal(Decimal.pow(Decimal.mul(i+1+i1*0.6,0.25).add(0.7), 166).mul(2));  
